...
/**
 * Created by caoliang on 16/7/12.
 */
public class NinePatchUtils {
    ...
    public static Drawable decodeDrawableFromFile(String filePath) throws Exception {
        Bitmap bitmap = decodeFromFile(filePath);
        if (bitmap.getNinePatchChunk() == null) {
            return new BitmapDrawable(bitmap);
        } else {
            Rect paddingRect = new Rect();
            readPaddingFromChunk(bitmap.getNinePatchChunk(), paddingRect);
            return new NinePatchDrawable(bitmap, bitmap.getNinePatchChunk(), paddingRect, null);
        }
    }
    /***
     * 通过流读取图片
